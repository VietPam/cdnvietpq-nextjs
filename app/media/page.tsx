...
export default function MediaPage() {
  ...
  // --- THUẬT TOÁN CÂN BẰNG LAYOUT (ZIG-ZAG) ĐÃ ĐƯỢC TỐI ƯU ---
  const displayData = useMemo(() => {
    if (!data?.data) return [];

    // 1. Tính toán Aspect Ratio dựa trên data Backend (H/W - tỉ lệ chiều dọc)
    // Ảnh càng dọc thì ratio càng lớn
    const withRatio = data.data.map(item => ({
      ...item,
      ratio: (item.height && item.width) ? (item.height / item.width) : 1
    }));

    // 2. Sắp xếp giảm dần (Ảnh cao nhất lên đầu)
    withRatio.sort((a, b) => b.ratio - a.ratio);

    // 3. Thuật toán Zig-Zag để rải đều các ảnh cao vào các cột khác nhau
    ...
    for (let i = 0; i < withRatio.length; i++) {
      ...
      let sortedIndex;
      if (row % 2 === 0) {
        // Dòng chẵn: Rải từ trái sang phải (0 -> 4)
        sortedIndex = base + col;
      } else {
        // Dòng lẻ: Rải ngược từ phải sang trái (4 -> 0) để bù đắp chiều cao cho cột ngắn
        sortedIndex = base + (itemsInCurrentRow - 1 - col);
      }
  ...
  // Hiển thị Skeleton mượt mà ngay khi API bắt đầu fetch
  if (isLoading) return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Skeleton width={250} height={40} sx={{ mb: 4, borderRadius: 2 }} />
      <Masonry columns={colCount} spacing={1.5}>
        {[350, 200, 450, 300, 400, 250, 350, 450, 200, 300].map((h, i) => (
          ...
          {displayData.map((item) => (
            <Card
              ...
              sx={{
                ...
                bgcolor: 'grey.100',
                // Duy trì tỉ lệ khung hình bằng CSS để tránh giật Masonry
                aspectRatio: `${item.width || 1} / ${item.height || 1}`,
                '&:hover .overlay-content': { opacity: 1 },
                '&:hover img, &:hover video': { transform: 'scale(1.05)' }
              }}
            >
              <Box sx={{ lineHeight: 0, height: '100%', overflow: 'hidden', transition: '0.4s' }}>
                <MediaRenderer
